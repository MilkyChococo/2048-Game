...
class Game2048:
    def __init__(self):
        ...
        self.board_values = [[0 for _ in range(8)] for _ in range(8)]
    ...
    def draw_over(self):
        pygame.draw.rect(self.screen, 'black', [200, 280, 300, 100], 0, 10)
        game_over_text1 = self.font.render('Game Over!', True, 'white')
        game_over_text2 = self.font.render('Press Enter to Restart', True, 'white')
        self.screen.blit(game_over_text1, (285, 300))
        self.screen.blit(game_over_text2, (225, 340))
                ...
                for j in range(8):
                    if board[i][j] != 0:
                        ...
                        if i - shift > 0 and board[i - shift - 1][j] == board[i - shift][j] and not merged[i - shift][j] and not merged[i - shift - 1][j]:
                            ...
                            move_made = True
            self.edge_spawn_blocked['UP'] = any(board[0][j] != 0 for j in range(8))
                ...
                for j in range(8):
                    if board[i][j] != 0:
                        ...
                        if shift > 0:
                            ...
                            board[i][j] = 0
                            move_made = True
                        if i + shift < 7 and board[i + shift + 1][j] == board[i + shift][j] and not merged[i + shift][j] and not merged[i + shift + 1][j]:
                            board[i + shift + 1][j] *= 2
                            self.ting.play()
                            self.score += board[i + shift + 1][j]
                            board[i + shift][j] = 0
                            merged[i + shift + 1][j] = True
                            move_made = True
            self.edge_spawn_blocked['DOWN'] = any(board[5][j] != 0 for j in range(8))
                ...
                for i in range(8):
                    if board[i][j] != 0:
                        ...
                        if j - shift > 0 and board[i][j - shift - 1] == board[i][j - shift] and not merged[i][j - shift] and not merged[i][j - shift - 1]:
                            ...
                            move_made = True
            self.edge_spawn_blocked['LEFT'] = any(board[i][0] != 0 for i in range(8))
                ...
                for i in range(8):
                    if board[i][j] != 0:
                        ...
                        if shift > 0:
                            ...
                            board[i][j] = 0
                            move_made = True
                        if j + shift < 7 and board[i][j + shift + 1] == board[i][j + shift] and not merged[i][j + shift] and not merged[i][j + shift + 1]:
                            board[i][j + shift + 1] *= 2
                            self.ting.play()
                            self.score += board[i][j + shift + 1]
                            board[i][j + shift] = 0
                            merged[i][j + shift + 1] = True
                            move_made = True
            self.edge_spawn_blocked['RIGHT'] = any(board[i][7] != 0 for i in range(8))
    ...
    #Tạo bảng trò chơi
    def draw_board(self):
        pygame.draw.rect(self.screen, self.colors['bg'], [0, 0, 700, 700], 0, 10)
        score_text = self.font.render(f'Score: {self.score}', True, 'black')
        high_score_text = self.font.render(f'High Score: {self.high_score}', True, 'black')
        self.screen.blit(score_text, (10, 710))
        self.screen.blit(high_score_text, (10, 750))
    ...
    def draw_pieces(self):
        for i in range(8):
            for j in range(8):
                ...
                color = self.colors[value] if value <= 8192 else self.colors['other']
                pygame.draw.rect(self.screen, color, [j * 85 + 10, i * 85 + 10, 65, 65], 0, 5)
                if value > 0:
                    value_len = len(str(value))
                    font = pygame.font.Font('freesansbold.ttf', 38 - (5 * value_len))
                    value_text = font.render(str(value), True, value_color)
                    text_rect = value_text.get_rect(center=(j * 85 + 44, i * 85 + 44))
                    self.screen.blit(value_text, text_rect)
                    pygame.draw.rect(self.screen, 'black', [j * 85 + 10, i * 85 + 10, 65, 65], 2, 5)
        ...
        while run:
            ...
            if self.game_over:
                if self.undo_count>0:
                    ...
                    self.screen.blit(second_chance1, (265, 300))
                    self.screen.blit(second_chance2, (245, 340))
                else:
                    self.draw_over()
